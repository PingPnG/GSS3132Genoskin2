...
housekeeping <- c("GAPDH", "PPIA", "ACTB")
...
meta<- data.frame(orgID,Time,  dup, trt, SID, trtTime, Tissue)
...
for (i in  1:d[1]){
  ...
  summary_testdata2 <- summary_testdata2 %>%
    group_by(trt) %>%
    mutate(ddct_shift = mean_ddct - mean_ddct[Time == 24]) %>%  # Shift all ddt values based on Time = 72
    ungroup()
...
print(doc, target = paste0(file, ".pptx"))
###################above this line, everything good, start to crash later####################################
...
#############################################################
avePCAData$Time <- factor(avePCAData$Time, levels = c(24, 72, 96), ordered = TRUE)
...
############################
combined_df$Time <- factor(combined_df$Time, levels = c(24, 72, 96), ordered = TRUE)
